...
% player goes second
        ...
        for j=1:playersCardsLength
            ...
            if (length(strfind(cardPlayed,firstElement))>0 || length(strfind(cardPlayed,secondElement))>0) && length(strfind(nextCardPlayed,"t"))==0
        ...
        if length(playersCardsLength)==length(playersCards) %MIGHT NEED TO CHANGE THIS SO THE PLAYER CAN PICK UP A CARD AFTER PLACING A +4 CARD
