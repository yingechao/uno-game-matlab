...
while length(usersCards)>0 && length(playersCards)>0 && length(cardDeck)>0
    ...
    while continueLoop
        ...
        elseif (length(strfind(nextCardPlayed,i))>0 || length(strfind(nextCardPlayed,i2))>0 && ismember(nextCardPlayed,usersCards)>0) && length(nextCardPlayed)==2 && length(strfind(nextCardPlayed,"t"))==0
            ...
            usersCards(index(1))=[];
            % if length(strfind(nextCardPlayed,"t"))>0
            ...
            %     %MOVE THIS SOMEWHERE SO THAT IT SKIPS THE PLAYERS TURN 
            % end
...
% player goes second
        playersCardsCopy=playersCards;
        for j=1:length(playersCards)
        ...
        if length(playersCardsCopy)==length(playersCards)
