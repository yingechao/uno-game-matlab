...
while gameOn
    ...
    if length(usersCards)>0 %MAYBE NEED THIS FOR USERS TURN, NOT TOO SURE THOUGH.
        ...
        indexValue=1;
            ...
            if playersCardsLength==playersCardsLength2
                if length(cardNumber)==0 || length(playersCards)==0 
                    break
                end
    ...
    %END OF GAME
    usersScore=string(usersScore);
    playersScore=string(playersScore);

    scoreBoard=[userName,usersScore;"Player",playersScore];
    
    if length(usersCards)==0
        fprintf("You have removed all cards in your hand. \nYOU WON!")
        fprintf("\n\nYou have scored 1 point. The current scores are: ")
        usersScore=str2num(usersScore);
        usersScore=usersScore+1;
        fprintf("\n\n%s's Score: %.0f",userName, scoreBoard(1,2))
        fprintf("\nPlayer Score: %.0f",scoreBoard(2,2))
    ...
    elseif length(playersCards)==0
        ...
        playersScore=str2num(playersScore);
        ...
        fprintf("\n\n%s's Score: %.0f",scoreBoard(1,1), usersScore)
        fprintf("\nPlayer Score: %.0f",usersScore) 
        playAgain=input("\n\nWould you like to play another round? (Enter Y/N): ","s");
