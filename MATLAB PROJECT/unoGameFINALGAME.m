...
while nameNotEntered
    userName=input("\n\nWhat is your name? ","s"); %get name input from user and store in variable userName
...
fprintf("\nPress any key to continue to each new instruction: ")
pause()
fprintf("\n\nThe rules of this game are like usual. You can only play a card that is the same colour (r, g, b, or y)\nor same number as the card on the deck.")
...
while repeatLegendRequest
    displayLegend=input("\nWould you like a legend to see the correct inputs for each card? Select Y (yes) or N (no): ","s");
    if displayLegend=="Y"
    ...
    while length(usersCards)>0 && length(playersCards)>0 && length(cardNumber)>0
        ...
        while usersTurn
            ...
            elseif nextCardPlayed=="C" %check if player has played a colour change card
                ...
                fprintf("\nYour cards are:")
                disp(usersCards)
                fprintf("What colour would you like to change to? \n")
                colour=input("Select either r (red), b (blue), g (green), or y (for yellow): ","s");
            ...
            elseif nextCardPlayed=="W"
                index=find(usersCards==nextCardPlayed);
                usersCards(index(1))=[];
                fprintf("\nYou played a +4 wild card! The opponent will automatically pick up 4 cards. ")
                fprintf("\nYour cards are:")
                disp(usersCards)
                colour=input("\nWhat colour would you like to change to? \nSelect either r (red), b (blue), g (green), or y (for yellow): ", "s");
        ...
        indexValue=1; %initiate index value for number of loop iterations 
            ...
            while playersTurn && indexValue<=playersCardsLength
                ...
                elseif (cardCompare(cardPlayed,nextCardPlayed))>0 && length(strfind(nextCardPlayed,"t"))>0
                    ...
                    indexValue=0; % resets index value so that the player loops through the array from the beginning after placing a +2 card
                    cardPlayed=nextCardPlayed;
                ...
                elseif (cardCompare(cardPlayed,nextCardPlayed))>0 && length(strfind(nextCardPlayed,"v"))>0
                    fprintf("\nThe player has played a reverse card. \nThe turn has reversed back to the player, skipping your turn. The player will go again: ")
                    index=find(playersCards==nextCardPlayed);
                    playersCards(index(1))=[]; 
                    playersCardsLength=playersCardsLength-1;
                    indexValue=0;
